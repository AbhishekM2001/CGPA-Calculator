...
public class CGPA_Calculator extends Application
{
  ...
  public void start(Stage primaryStage) throws Exception
  {
    ...
    grid.setVgap(8);
    grid.setHgap(10);//Course Label - constrains use (child, column, row)
    Label nameLabel = new Label("#");
    GridPane.setConstraints(nameLabel, 0, 0); //Grade Label
    Label gradeLabel = new Label("GPA:");
    GridPane.setConstraints(gradeLabel, 1, 0);
    Label credLabel = new Label("Semester Credits:"); GridPane.setConstraints(credLabel, 2, 0);
    grid.getChildren().addAll(nameLabel, gradeLabel, credLabel);//adding labels Label sem_no ;
    TextField gradeInputs[] = new TextField[max_num_of_courses];
    TextField credInputs[] = new TextField[max_num_of_courses];
    for(int i=0 ; i<max_num_of_courses-1 ;
    {
      gradeInputs[i] = new TextField(); GridPane.setConstraints(gradeInputs[i],1,i+1);
      credInputs[i] = new TextField();
      GridPane.setConstraints(credInputs[i],2,i+1);
      sem_no = new Label(Integer.toString(i+1)); GridPane.setConstraints(sem_no,0,i+1); grid.getChildren().addAll(sem_no,gradeInputs[i],credInputs[i]);
    }
    Button calcButton = new Button("Calculate"); GridPane.setConstraints(calcButton, 1,max_num_of_courses+1 );
  ...
  public static void store_into_database(TextField[] gradeInputs,TextField[] credInputs)
  {
    ...
    try
    {
      String query = "insert into grades values (?,?)"; //query to store value Class.forName("com.mysql.jdbc.Driver");
      Connection con = DriverManager.getConnection(URL,uname,pass); con.createStatement().executeUpdate("truncate table grades"); // Clear table contents
      PreparedStatement st = con.prepareStatement(query);
      for(int i=0;i<max_num_of_courses-1;i++)
      {
      if(gradeInputs[i].getText().trim().equals("") ||credInputs[i].getText().trim().equals("") )
      continue; //storing grade(GPA) and credits as strings st.setString(1, gradeInputs[i].getText());
  ...
  public static String calculate() throws Exception
  {
    ...
    String URL = "jdbc:mysql://localhost:3306/demo_jdbc";
    String uname = "root"; String pass = "srikar123"; String query = "insert into grades values (?,?)"; //query to store value Class.forName("com.mysql.jdbc.Driver");
    Connection con = DriverManager.getConnection(URL,uname,pass); Statement stat = con.createStatement();
